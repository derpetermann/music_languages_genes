---
title: "Supporting Information: Relationships between cultural and genetic evolution in Northeast Asian history"
author: 
  - Hiromi Matsumae
  - Peter Ranacher
  - Patrick E. Savage 
...
pcos_to_factors <- function(var_th, genetics, genetics_ev, grammar, grammar_ev, 
                            phonology, phonology_ev, music, music_ev, geo){
  ...
  grammar_pc_rel <- grammar[, which(grammar_ev >= var_th*100)]
...
add_proportion_and_kld <- function(rda_flat, diff_th){
  ...
  # to the permuted (py) distribution
...
association_plot_partial <- function (prop, n_sites=NULL, k=F, title=NULL){
  ...
  assoc <- as.character(levels(prop$rda_df$val$Association))
  if (is.null(n_sites)){
    if (isTRUE(k)){
  ...
  p <- ggplot(prop$rda_df$z_val, aes(x = z, y=Association, fill=kld)) +
    stat_density_ridges(geom = "density_ridges_gradient", scale=0.6, size=.2) +
    scale_fill_gradient(low = "gray95", high = "black", name = "KLD", 
                        limits = c(0,max(prop$rda_df$z_val$kld))) +
    geom_vline(xintercept = 0, size=.6) +
    geom_vline(xintercept = 1, size=.6, linetype = "dashed") +
    annotate("text", x = 0.4, y = 1.7:(length(prop_pos_labels) + 0.7), size=2.5,
             label = prop_pos_labels) +
    annotate("text", x = 0.5, y = 0.6, size=3,
             label = expression(P(z > 0))) + 
...
We load Levenshtein distances derived from the ASJP data [@Wichmannetal2015The-ASJP]:
...
| Evenk         | 15           | 28                | @Rasmussen2010                         | Evenki                    | [i-evn][a-527][g-even1259]  |
...
The geographical polygon locations of the languages are taken from the Ethnologue [@ethnologue2018]; as there is no polygon for Ainu, we drew one ourselves.

The split into phonology vs. grammar is based on the broad definition in `categorization_of_variables.csv` which includes phonotactic and morphophonological data under phonology.^[Available at  https://www.geo.uzh.ch/microsite/MusicGenesLanguages] We extract a subset from the data comprising languages from fourteen different societies. Note that in AUTOTYP and WALS, Buriat is represented by ISO code `bua` (Buriat in general), while in PHOIBLE  it is represented by ISO code  `bxr` and in the ANU data by `bxm`. We map all of them below to `bua`.
...
In view of the fact that the grammar and phonology data are partly numerical and partly categorical, we use a balanced mix of PCA and MCA [@Leetal2008FactoMineR] to bring out the main patterns in the data. Empty values are imputed using the methods developed by @Josseetal2016missMDA.
...
```{r screegenetics, fig.cap='\\label{fig:figs}Scree plot of explained variance for Genetics', fig.path='figures/', dev='cairo_pdf', echo=F, cache=T, fig.width=8, out.extra=''}
...
```{r screemusic, fig.cap='\\label{fig:figs}Scree plot of explained variance for Music', fig.path='figures/', dev='cairo_pdf', fig.width=7, cache=T, echo=F}
plot_pc_scree(eigenval=music_ev, title="Music", type="PCo")
```

```{r screegrammar,  fig.cap='\\label{fig:figs}Scree plot of explained variance for Grammar', fig.path='figures/', fig.width=7, dev='cairo_pdf', cache=T, echo=F}
plot_pc_scree(eigenval=grammar_ev, title="Grammar", type="PC")
```

```{r screephon, fig.cap='\\label{fig:figs}Scree plot of explained variance for Phonology', fig.path='figures/', fig.width=7, dev='cairo_pdf', cache=T, echo=F}
...
```{r heatgenetics, fig.cap='\\label{fig:figs} Heat plot of the first nine PCos (normalized) of Genetics', fig.path='figures/', dev='cairo_pdf', fig.width=7, fig.height=6, echo=F, fig.pos='H', cache=T}
heat_map_pc(genetics_pco, type="PCo")
```


```{r heatmusic, fig.cap='\\label{fig:figs} Heat plot of the first six PCos (normalized) of Music', fig.path='figures/', dev='cairo_pdf', fig.width=6, fig.height=6, cache=T, echo=F}
heat_map_pc(music_pco, type="PCo")
```


```{r heat_map_grammar, fig.cap='\\label{fig:figs} Heat plot of the first ten PCs (normalized) of Grammar', fig.path='figures/', dev='cairo_pdf', fig.width=7, fig.height=7, cache=T, echo=F}
heat_map_pc(grammar_pc, type="PC")
```


```{r heatphon, fig.cap='\\label{fig:figs} Heat plot of the first ten PCs (normalized) of Phonology', fig.path='figures/', dev='cairo_pdf', fig.width=7, fig.height=7, cache=T, echo=F}
...
```{r lex_nnets, echo=F, eval=T, cache=T}

# Lexic
lex_nnet <- neighborNet(sort_dist_mat(lex_dist) )

```

```{r gen_nnets, echo=F, eval=T, cache=T}
# Genetics
genetics_nnet <- neighborNet(sort_dist_mat(dist_list$genetics))


```

```{r music_nnets, echo=F, eval=T, cache=T}
# Music
music_nnet <- neighborNet(sort_dist_mat(dist_list$music))

```

```{r gram_nnets, echo=F, eval=T, cache=T}
# Grammar
grammar_nnet <- neighborNet(sort_dist_mat(dist_list$grammar))

```

```{r, phon_nnets, echo=F, eval=T, cache=T}
# Phonology
phonology_nnet <- neighborNet(sort_dist_mat(dist_list$phonology))
```


```{r lexdist, fig.cap='Lexical (ASJP) distances (Nivkh and Nganasan missing)', echo=T, fig.pos='H', dev='cairo_pdf', dev='cairo_pdf', cache=T, fig.width=14, fig.height=16, out.extra='',out.width='.8\\linewidth',  fig.path='figures/'}
print_dist(lex_dist, 'Lexical distances')
plot(lex_nnet, "2D", show.edge.label = F, edge.width=1, cex=0.8)
```
\clearpage


```{r gendist, fig.cap='Genetic distances', echo=F, fig.pos='H', dev='cairo_pdf', cache=T, fig.width=14, fig.height=16, out.extra='',out.width='.8\\linewidth', fig.path='figures/'}
print_dist(dist_list$genetics, 'Genetic distances')
plot(genetics_nnet, "2D", show.edge.label = F, edge.width=1, cex=0.7)
```
\clearpage

```{r musicdist, fig.cap='Music distances', echo=F, fig.pos='H', dev='cairo_pdf', cache=T, fig.width=14, fig.height=16, out.extra='', fig.path='figures/', out.width='.9\\linewidth'}
print_dist(dist_list$music, 'Music distances ')
set.seed(45)
plot(music_nnet, "2D", show.edge.label = F, edge.width=1, cex=0.9)
```


```{r gramdist, fig.cap='Grammar distances (scaled)', echo=F, dev='cairo_pdf', fig.pos='H', cache=T, fig.width=14, fig.height=16, out.extra='', fig.path='figures/'}
print_dist(dist_list$grammar, 'Grammar distances')
set.seed(7)
plot(grammar_nnet, "2D", show.edge.label = F, edge.width=1, cex=0.9)
```


```{r phondist, fig.cap='Phonology distances (scaled)', echo=F, dev='cairo_pdf', fig.pos='H', cache=T, fig.width=14, fig.height=16, out.extra='', out.width='.8\\linewidth', fig.path='figures/'}
print_dist(dist_list$phonology, 'Phonology distances')
set.seed(11)
plot(phonology_nnet, "2D", show.edge.label = F, edge.width=1, cex=0.9)
```
...
\begin{itemize}
  ...
  \item {\textbf{Setup 2: Partial RDA controlling for spatial autocorrelation and genealogy}: In addition to spatial neighborhood we also control for genalogy. We perform a partial RDA for random samples of non-related languages only and explore to what degree the associations are driven by phylogenetic inheritance.}
\end{itemize}
...
\begin{itemize}
  ...
  \item {\textbf{Senisitivity 3: Spatial distribution of $R^2$} We explore the spatial locations of samples for which the adjusted $R^2$ is particularily low/high. If locations with low and high (adjusted) $R^2$ differ in space, this might indicate that a particular neighborhood scenario (low $R^2$) explains the variance in the response, while another one does not (high $R^2$).}
\end{itemize}
...
```{r rda0plot, fig.cap='\\label{fig:figs} RDA: Variance in the response explained by each explanatory variable;  * indicates a  significant association ($p \\leq 0.05$).', fig.path='figures/', dev='cairo_pdf', cache=T, fig.width=10, fig.height=7}
...
The two density plots belwo show the difference between observed and permuted adjusted $R^2$ for the association between genetics and grammar and between grammar and genetics. 


```{r gengramrda1, fig.cap='\\label{fig:figs} Partial RDA controlling for spatial autocorrelation: The association between Genetics (explanatory variable) and Grammar (response). In the figure on the left, the red curve shows the observed (adjusted) $R^2$, the blue curve the $R^2$ for random permutations. In the figure on the right, the green curve shows the difference of $R^2$ (observed - random permutations).', fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, echo=F, cache=T}
...
- all samples with positive z-scores ($P(z>0)$ SD), i.e. samples where the observed adjusted $R^2$ is larger than the permuted
...
The KLD allows to assess the overall divergence of the two distributions, $P(z>0)$ SD and $P(z>1)$ SD report the the proportion of (strongly) positive differences.

```{r, r2means1, cache=T}

rda_setup_1_z <- partial_rda_to_z_val(rda_setup_1, r2_type="r2_adj_partial")
proportion_rda_setup_1 <- add_proportion_and_kld(rda_setup_1_z, diff_th = 1.)
```

We visualize the difference between observed and permuted adjusted $R^2$ in a ridge plot. 

```{r ridge1, echo=F, fig.cap='Partial RDA controlling for spatial autocorrelation: Densities of the difference between observed and permuted adjusted $R^2$ (z-normalized). All input components contribute at least 15% to the explained variance. Numbers right to the solid and dashed line show the proportion of samples with a positive difference ($P(z>0$ SD) and a strong positive difference ($P(z>1$ SD), respectively. Grey shading reflects the Kullback-Leibler divergence (KLD) between the observed and permuted adjusted $R^2$.' , message=F, warning=F, dev='cairo_pdf', fig.path='figures/', fig.width=10, fig.height=7, echo=F, cache=T, fig.scap='Correlations based on 15%-components'} 
...
```{r gengramrda2, fig.cap='\\label{fig:figs} Partial RDA controlling for spatial autocorrelation and genealogy: The association between Genetics (explanatory variable) and Grammar (response). In the figure on the left, the red curve shows the observed (adjusted) $R^2$, the blue curve the $R^2$ for random permutations. In the figure on the right, the green curve shows the difference of $R^2$ (observed - random permutations).', fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, echo=F, cache=T}
...
```{r gramgenrda2, fig.cap='\\label{fig:figs} Partial RDA controlling for spatial autocorrelation and genealogy: The association between Grammar (explanatory variable) and Genetics (response). In the figure on the left, the red curve shows the observed (adjusted) $R^2$, the blue curve the $R^2$ for random permutations. In the figure on the right, the green curve shows the difference of $R^2$ (observed - random permutations).', fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, message=FALSE, warning=FALSE, echo=F, cache=T}
...
We statistically evaluate the difference between the observed and the permuted adjusted $R^2$ and visualize it in a ridge plot.

```{r, r2means2, cache=T}

rda_setup_2_z <- partial_rda_to_z_val(rda_setup_2, r2_type="r2_adj_partial")
proportion_rda_setup_2 <- add_proportion_and_kld(rda_setup_2_z, diff_th = 1.)
```


```{r ridge2, echo=F, fig.cap='Partial RDA controlling for spatial autocorrelation and genealogy: Densities of the difference between observed and permuted adjusted $R^2$ (z-normalized) in the partial RDA. All input components contribute at least 15% to the explained variance. Numbers right to the solid and dashed line show the proportion of samples with a positive difference ($P(z>0$ SD) and a strong positive difference ($P(z>1$ SD), respectively. Grey shading reflects the Kullback-Leibler divergence (KLD) between the observed and permuted adjusted $R^2$.', message=F, warning=F, dev='cairo_pdf', fig.path='figures/', fig.width=10, fig.height=7, echo=F, cache=T, fig.scap='Correlations based on 15%-components'} 
...
```{r ridgeplot1 sens pc , echo=F, fig.cap='The influence of the number of principal components/coordinates on the association between genetics (explanatory variable) and grammar (response). PCs/Pcos must account for at least $k$% of the explained variance. Densities of the difference between observed and permuted adjusted $R^2$ (z-normalized) in the partial RDA. Numbers right to the solid and dashed line show the proportion of samples with a positive difference ($P(z>0$ SD) and a strong positive difference ($P(z>1$ SD), respectively. Grey shading reflects the Kullback-Leibler divergence (KLD) between the observed and permuted adjusted $R^2$.', message=F, warning=F, dev='cairo_pdf', fig.path='figures/', fig.width=10, fig.height=7, cache=T, echo=F} 
# Genetics --> Grammar 
...
```{r ridgesens1 , echo=F, fig.cap='The influence of the number of principal components/coordinates on the association between grammar (explanatory variable) and genetics (response). PCs/Pcos must account for at least $k$% of the explained variance. Densities of the difference between observed and permuted adjusted $R^2$ values (z-normlalized) in the partial RDA.  Numbers right to the solid and dashed line show the proportion of samples with a positive difference ($P(z>0$ SD) and a strong positive difference ($P(z>1$ SD), respectively. Grey shading reflects the Kullback-Leibler divergence (KLD) between the observed and permuted adjusted $R^2$.', message=F, warning=F, dev='cairo_pdf', fig.path='figures/', fig.width=10, fig.height=7, cache=T, echo=F} 
# Grammar --> Genetics
...
for (n in names(rda_sens_sites_1)[names(rda_sens_sites_1) != "n_sites"]){
    ...
    proportion_rda_sens_sites_1[[n]] <- add_proportion_and_kld(z, diff_th = 1.)}
...
for (n in names(rda_sens_sites_2)[names(rda_sens_sites_2) != "n_sites"]){
    ...
    proportion_rda_sens_sites_2[[n]] <- add_proportion_and_kld(z, diff_th=1.)}
    
```


```{r ridge1sens2gengra, echo=F, fig.cap='The influence of single societies on the association between genetics (explanatory variable) and grammar (response). Densities of the difference between observed and permuted adjusted $R^2$ (z-normalized) in the partial RDA controlling for space but not for genealogy. Numbers right to the solid and dashed line show the proportion of samples with a positive difference ($P(z>0$ SD) and a strong positive difference ($P(z>1$ SD), respectively. Grey shading reflects the Kullback-Leibler divergence (KLD) between the observed and permuted adjusted $R^2$.', message=F, warning=F, dev='cairo_pdf', fig.path='figures/', fig.width=10, fig.height=7, cache=T, echo=F}
# Genetics --> Grammar 
...
```{r ridge2sens2gengra, echo=F, fig.cap='The influence of potential outlier societies on the association between genetics (explanatory variable) and grammar (response). Densities of the difference between observed and permuted adjusted $R^2$ (z-normalized) in the partial RDA controlling for space and for genealogy. Numbers right to the solid and dashed line show the proportion of samples with a positive difference ($P(z>0$ SD) and a strong positive difference ($P(z>1$ SD), respectively. Grey shading reflects the Kullback-Leibler divergence (KLD) between the observed and permuted adjusted $R^2$.', message=F, warning=F, dev='cairo_pdf', fig.path='figures/', fig.width=10, fig.height=7, cache=T, echo=F}
# Genetics --> Grammar 
...
```{r ridge1sens2gragen, echo=F, fig.cap='The influence of potential outlier societies on the association between grammar (explanatory variable) and genetics (response). Densities of the difference between observed and permuted adjusted $R^2$ (z-normalized) in the partial RDA controlling for space but not for genealogy. Numbers right to the solid and dashed line show the proportion of samples with a positive difference ($P(z>0$ SD) and a strong positive difference ($P(z>1$ SD), respectively. Grey shading reflects the Kullback-Leibler divergence (KLD) between the observed and permuted adjusted $R^2$.', message=F, warning=F, dev='cairo_pdf', fig.path='figures/', fig.width=10, fig.height=7}
...
```{r ridge2sens2gragen, echo=F, fig.cap='The influence of potential outlier societies on the association between grammar (explanatory variable) and genetics (response). Densities of the difference between observed and permuted adjusted $R^2$ (z-normalized) in the partial RDA. Numbers right to the solid and dashed line show the proportion of samples with a positive difference ($P(z>0$ SD) and a strong positive difference ($P(z>1$ SD), respectively. Grey shading reflects the Kullback-Leibler divergence (KLD) between the observed and permuted adjusted $R^2$.', message=F, warning=F, dev='cairo_pdf', fig.path='figures/', fig.width=10, fig.height=7, cache=T, echo=F}
# Grammar --> Genetics
...
###  Senisitivity 3: Spatial distribution of $R^2$

Spatial neighborhood is unlikely to explain the observed relationship between factors if the adjusted $R^2$ for all spatial locations is well above zero and if it differs from the adjusted $R^2$ under random permutations, both of which we have shown above. However, the correlations we find might still be an artefact of space, in particular if locations with low adjusted $R^2$ (for example the lower tail of observed distributions in Figures S21) cluster together in different parts of the language polygons than those with particularly high adjusted $R^2$ (e.g. the upper tail of observed distributions in Figures S21) . To rule this out, we map all samples for which the adjusted $R^2$ is in the .2 percentile and the 0.8 percenile, respectivily, and compare both spatial patterns. The following figures suggest that for both the association between grammar and genetics and genetics and grammar locations with low and high $R^2$ randomly distribute in the polygons and do not mass up in specific regions. 


```{r lowrdagengram, fig.cap='Location samples used for removing the influence of space in the partial RDA between Genetics (explanatory variable) and Grammar (response) with a low adjusted $R^2$', fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, echo=F, cache=T, fig.pos='H', out.extra=''}
...
```{r highrdagengram, fig.cap='Location samples used for removing the influence of space in the partial RDA between Genetics (explanatory variable) and Grammar (response) with a high adjusted $R^2$', fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, echo=F, cache=T, fig.pos='H', out.extra=''}
...
```{r lowrdagramgen, fig.cap='Location samples used for removing the influence of space in the partial RDA between Grammar (explanatory variable) and Genetics (response) with a low adjusted $R^2$', fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, echo=F, cache=T, fig.pos='H', out.extra=''}
...
```{r highrdagramgen, fig.cap='Location samples used for removing the influence of space in the partial RDA between Grammar (explanatory variable) and Genetics (response) with a high adjusted $R^2$', fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, echo=F, cache=T, fig.pos='H', out.extra=''}

---
title: "Supporting Information: Exploring correlations in genetic and cultural variation across language families in Northeast Asia"
author: 
  - Hiromi Matsumae
  - Peter Ranacher
  - Patrick E. Savage 
...
lex_dist <- sort_dist_mat(as.dist(
  read.csv('data/lexicon/ASJP13PopDist.csv', header=T, row.names = 1)))
...
Lexical distances are visualized directly. For nonlexical data, we compute Euclidean distances from the dimensionality-reduced data. All NeighborNets were created with SplitsTree [@huson2006application]. 
...
```{r lexicaldist, fig.cap='Lexical (ASJP) distances (Nganasan missing)', echo=F, fig.align="center", fig.pos='H', cache=F, fig.width=14, fig.height=16, out.extra='', out.width='.6\\linewidth'}
...
```{r ridge1, fig.cap='Partial RDA controlling for spatial autocorrelation: Densities of the difference between observedand permuted adjusted $R^2$ (z-normalized). All input components contribute at least 15\\% to the explained variance. Numbers right to the solid and dashed line show the proportion of samples with a positive difference \u2013 $P(z>0)$ \u2013 and a strong positive difference \u2013 $P(z>1$ SD), respectively. Grey shading reflects the Kullback-Leibler divergence (KLD) between the observed and permuted adjusted $R^2$.', fig.scap='Partial RDA controlling for space: Densities of all associations',  fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, message=FALSE, warning=FALSE, echo=F, cache=T, out.extra=''}
